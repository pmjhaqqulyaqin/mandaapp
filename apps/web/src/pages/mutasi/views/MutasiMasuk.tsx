...
export const MutasiMasuk = () => {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => apiClient<any[]>(`/mutations?type=masuk${searchQuery ? `&search=${searchQuery}` : ''}`).then(res => res),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient(`/mutations/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['mutations-masuk'] });
      ...
      <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-[#222] p-4 shadow-sm flex flex-col sm:flex-row justify-between gap-3">
        <div className="flex-1 max-w-md relative">
          ...
          <input 
            ...
            placeholder="Cari siswa mutasi masuk..."
            value={searchQuery} 
            onChange={e => setSearchQuery(e.target.value)} 
          />
        </div>
        <Button onClick={() => { setEditData(null); setIsModalOpen(true); }} className="flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-[#222] shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            <ArrowDownToLine size={48} className="mx-auto text-emerald-200 dark:text-emerald-900/50 mb-3" />
            <p className="text-sm font-semibold text-text-primary dark:text-gray-400">Tidak ada data mutasi masuk</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left whitespace-nowrap">
              <thead>
                <tr className="border-b border-gray-100 dark:border-[#222] text-[11px] uppercase tracking-wider text-text-secondary bg-gray-50/50 dark:bg-[#0a0a0a]">
                  ...
                  <th className="py-3 px-4 font-semibold">Sekolah Asal</th>
                  <th className="py-3 px-4 font-semibold">Masuk Kelas</th>
              ...
              <tbody>
                {records.map((r: any) => (
                  <tr key={r.id} className="border-b border-gray-50 dark:border-[#1a1a1a] hover:bg-gray-50/50 dark:hover:bg-[#0a0a0a]">
                    ...
                    <td className="py-3 px-4 text-sm">{r.fromSchool || '-'}</td>
