...
import { Search, Plus, Trash2, Edit, Loader2, ArrowUpFromLine } from 'lucide-react';
import { MutasiFormModal } from '../components/MutasiFormModal';
import { DataTableToolbar } from '../../../components/DataTableToolbar';

export const MutasiKeluar = () => {
  ...
  const { data: records = [], isLoading } = useQuery({
    queryKey: ['mutations-keluar', searchQuery],
    queryFn: () => apiClient<any[]>(`/mutations?type=keluar${searchQuery ? `&search=${searchQuery}` : ''}`).then(res => res),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient(`/mutations/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['mutations-keluar'] });
  ...
  return (
    <div className="flex flex-col gap-4 animate-in fade-in duration-500 pb-10">
      <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-[#222] p-4 shadow-sm flex flex-col sm:flex-row justify-between gap-3">
        ...
        <Button onClick={() => { setEditData(null); setIsModalOpen(true); }} className="flex items-center gap-2 bg-rose-600 hover:bg-rose-700">
          <Plus size={16} /> Tambah Data Keluar
        </Button>
      </div>
      ...
      <DataTableToolbar
        ...
        columns={[
          ...
          { header: 'Sekolah Tujuan', key: 'toSchool', transform: (v) => v || '-' },
          { header: 'Alasan', key: 'reason', transform: (v) => v || '-' },
          { header: 'Tanggal Efektif', key: 'effectiveDate', transform: (v) => v ? new Date(v).toLocaleDateString('id-ID') : '-' },
        ]}
        fileName="Mutasi_Keluar"
      ...
      <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-[#222] shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            <ArrowUpFromLine size={48} className="mx-auto text-rose-200 dark:text-rose-900/50 mb-3" />
            <p className="text-sm font-semibold text-text-primary dark:text-gray-400">Tidak ada data mutasi keluar</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left whitespace-nowrap">
              <thead>
                <tr className="border-b border-gray-100 dark:border-[#222] text-[11px] uppercase tracking-wider text-text-secondary bg-gray-50/50 dark:bg-[#0a0a0a]">
                  ...
                  <th className="py-3 px-4 font-semibold">Sekolah Tujuan</th>
                  <th className="py-3 px-4 font-semibold">Alasan</th>
                  <th className="py-3 px-4 font-semibold">Tanggal Efektif</th>
              ...
              <tbody>
                {records.map((r: any) => (
                  <tr key={r.id} className="border-b border-gray-50 dark:border-[#1a1a1a] hover:bg-gray-50/50 dark:hover:bg-[#0a0a0a]">
                    ...
                    <td className="py-3 px-4 text-sm font-semibold">{r.toSchool || '-'}</td>
