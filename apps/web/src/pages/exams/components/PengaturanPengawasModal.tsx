...
import { X, UserPlus, Trash2, Hash, Type, Users } from 'lucide-react';
...
export const PengaturanPengawasModal = ({ isOpen, onClose, ujian, allEmployees, onSuccess }: Props) => {
  ...
  const [jumlahPengawas, setJumlahPengawas] = useState<1 | 2>(2);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setJumlahPengawas(config.jumlahPengawasPerRuang === 1 ? 1 : 2);
    }
  }, [isOpen, ujian]);
  ...
  const handleSave = async () => {
    try {
      const newPengaturan = {
        ...(ujian.pengaturan || {}),
        pengawasGroups: { group1, group2: jumlahPengawas === 2 ? group2 : [] },
        jumlahPengawasPerRuang: jumlahPengawas
      };
  ...
  const filteredEmployees = (allEmployees || []).filter(e => {
    ...
    const isAlreadyInGroup = group1.includes(e.id) || (jumlahPengawas === 2 && group2.includes(e.id));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-[#0a0a0a] rounded-xl border border-gray-200 dark:border-[#222] w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        {/* Jumlah Pengawas Toggle */}
        <div className="px-4 pt-4 pb-2">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-[#222]">
            <Users size={16} className="text-indigo-500 shrink-0" />
            <div className="flex-1">
              <p className="text-xs font-bold text-text-primary dark:text-text-darkPrimary">Jumlah Pengawas per Ruang</p>
              <p className="text-[10px] text-gray-400">Tentukan berapa pengawas yang ditugaskan di setiap ruang ujian</p>
            </div>
            <div className="flex items-center gap-1 bg-white dark:bg-[#111] rounded-lg border border-gray-200 dark:border-[#333] p-0.5">
              <button
                onClick={() => setJumlahPengawas(1)}
        ...
        <div className={`p-4 flex-1 overflow-auto grid gap-6 ${jumlahPengawas === 2 ? 'grid-cols-1 md:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'}`}>
          {/* Employee Pool */}
          ...
          <div className="flex flex-col h-full">
            <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-3 flex items-center gap-1">
              <Hash size={12} /> {jumlahPengawas === 1 ? 'Kelompok Pengawas (Angka)' : 'Kelompok I (Angka)'}
            </p>
            <div className="flex-1 bg-indigo-50/30 dark:bg-indigo-900/5 rounded-xl border border-dashed border-indigo-200 dark:border-indigo-800/30 p-2 space-y-1 overflow-auto max-h-[400px]">
              ...
              {group1.length === 0 && <p className="text-[10px] text-gray-400 text-center py-10 italic">{jumlahPengawas === 1 ? 'Belum ada pengawas' : 'Belum ada pengawas I'}</p>}
            </div>
          </div>

          {/* Group 2 (Alphabetic) - Only shown when 2 pengawas mode */}
          {jumlahPengawas === 2 && (
            <div className="flex flex-col h-full">
