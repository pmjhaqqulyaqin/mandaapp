...
export default function DashboardMutasi() {
  ...
  const filtered = useMemo(() => {
    return students.filter(s => {
      ...
      const matchStatus = status === 'pindah' || status === 'keluar' || status === 'do' || status === 'mutasi';
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          <Breadcrumbs items={[
            ...
            { label: activeTab === 'students' ? 'Data Mutasi' : 'Kelas' },
          ]} />
          <h1 className="text-xl font-bold text-text-primary dark:text-text-darkPrimary mt-1">Data Siswa Mutasi / Keluar</h1>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className={`bg-white dark:bg-[#111] rounded-xl border border-gray-200 dark:border-[#222] p-3 flex items-center gap-2.5 shadow-sm`}>
              <div className={`w-8 h-8 rounded-lg bg-red-500/10 flex items-center justify-center shrink-0`}><AlertCircle size={16} className="text-red-500" /></div>
              <div>
                <p className="text-[9px] font-semibold uppercase tracking-wider text-text-secondary">Total Mutasi / Keluar</p>
                <p className="text-base font-bold text-text-primary dark:text-text-darkPrimary leading-tight">{stats.mutasi.toLocaleString()}</p>
              </div>
            </div>
          </div>
        </>
