...
import { Button, Input, Badge, DataTable, Skeleton, Modal } from '@mandaapp/ui';
  ...
  const joditConfig = useMemo(() => ({
    ...
    uploader: {
      ...
      defaultHandlerSuccess: function(this: any, data: any) {
        ...
        const fileName = fileUrl.split('/').pop() || 'Dokumen_Terlampir';
        ...
        if (isImage) {
          ...
          const pdfHtml = `
            <div class="pdf-container" style="margin: 20px 0;">
              ...
              <iframe class="jodit_iframe" src="${fileUrl}" width="100%" height="700" style="border: 2px solid #e2e8f0; border-radius: 8px; display: block; background: #e5e5e5;"></iframe>
            </div>
            <p><br></p>`;
          this.selection.insertHTML(pdfHtml);
        } else {
          const genericHtml = `
            <a class="manda-file-card" href="${fileUrl}" target="_blank" download="${fileName}" style="display: inline-block; padding: 12px 20px; margin: 16px 0; border: 1px solid #cbd5e1; border-radius: 8px; background-color: #f8fafc; text-decoration: none; color: #0f172a; font-family: Inter, sans-serif;">
              <strong>📁 ${fileName}</strong>
              <div style="font-size: 12px; color: #64748b; margin-top: 4px;">Klik untuk mengunduh dokumen</div>
            </a>
            <p><br></p>`;
          this.selection.insertHTML(genericHtml);
        }
      }
    },
    buttons: [
