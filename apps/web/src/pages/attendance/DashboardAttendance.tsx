          ...
          <div className="flex overflow-x-auto no-scrollbar">
            <div className="inline-flex w-full md:w-auto gap-1 p-0.5 bg-gray-200/70 dark:bg-[#1a1a1a] rounded-xl">
              {([
                ...
                { key: 'scanner', icon: <Camera size={15} />, label: 'Scanner' },
                { key: 'manual', icon: <QrCode size={15} />, label: 'Manual' },
                { key: 'rekap', icon: <Grid size={15} />, label: 'Rekap' },
                { key: 'settings', icon: <Settings size={15} />, label: 'Setting' },
              ] as const).map((tab) => (
                <button
                  key={tab.key}
